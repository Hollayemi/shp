...
function getEncryptionSecret(): string {
  ...
  if (!secret) {
    throw new Error(
      "CONVEX_DEPLOY_KEY_ENCRYPTION_SECRET environment variable is required"
    );
  }
  return secret;
}
 ...
 * ```
 */
export function encryptDeployKey(deployKey: string): string {
 ...
 * ```
 */
export function decryptDeployKey(encryptedKey: string): string {
  const secret = getEncryptionSecret();
 ...
 */
export function isValidDeployKeyFormat(deployKey: string): boolean {
 ...
 */
export function getDeployKeyType(
  deployKey: string
): "dev" | "prod" | "preview" | "project" | null {
  const pipeIndex = deployKey.indexOf("|");
  if (pipeIndex === -1) {
    return null;
  }
  ...
  if (["dev", "prod", "preview", "project"].includes(type)) {
    return type as "dev" | "prod" | "preview" | "project";
  }
 ...
 */
export function getDeploymentNameFromKey(deployKey: string): string | null {
  const pipeIndex = deployKey.indexOf("|");
  if (pipeIndex === -1) {
    return null;
  }
