 ...
 */
import { createConvexClient } from "./management-api.js";
...
export class ShipperCloudDeploymentService {
     ...
     * ```
     */
    async provisionBackend(shipperProjectId, projectName) {
        try {
            ...
            console.log(`[ShipperCloud] Creating Convex project: ${projectName} for Shipper project: ${shipperProjectId}`);
            const project = await this.client.createProject(projectName, "prod");
            console.log(`[ShipperCloud] Created project: ${project.deploymentName} at ${project.deploymentUrl}`);
            // Step 2: Create deploy key
            console.log(`[ShipperCloud] Creating deploy key for deployment: ${project.deploymentName}`);
            const keyName = `shipper-${shipperProjectId}-${Date.now()}`;
            const deployKey = await this.client.createDeployKey(project.deploymentName, keyName);
            ...
            console.log(`[ShipperCloud] Backend provisioned successfully for ${shipperProjectId}`);
            return {
                success: true,
            ...
            console.error(`[ShipperCloud] Failed to provision backend for ${shipperProjectId}:`, error);
            return {
                success: false,
            ...
            return new Promise((resolve) => {
                ...
                child.on("close", (code) => {
                    ...
                    if (code === 0) {
                        ...
                        const urlMatch = stdout.match(/https:\/\/[a-z-]+\d*\.convex\.cloud/);
                        resolve({
                            success: true,
     ...
     */
    async deleteDeployment(convexProjectId) {
