...
# Verify that essential shadcn components were installed correctly
RUN test -f ./src/components/ui/button.tsx || (echo "ERROR: button.tsx not found" && exit 1)
RUN test -f ./src/components/ui/card.tsx || (echo "ERROR: card.tsx not found" && exit 1) 
RUN test -f ./src/components/ui/input.tsx || (echo "ERROR: input.tsx not found" && exit 1)
RUN test -f ./src/lib/utils.ts || (echo "ERROR: utils.ts not found" && exit 1)

# Verify that components.json was created
RUN test -f ./components.json || (echo "ERROR: components.json not found" && exit 1)
...
# Clean up build artifacts to keep image size down
RUN rm -rf ./dist
