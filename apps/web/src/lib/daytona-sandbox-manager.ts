...
import { getRecursiveFileList } from "@/lib/ai/validation-utils";
import { runCommandOnSandbox } from "@/lib/ai/sandbox-compat";
import { prisma } from "@/lib/db";
import { TRPCError } from "@trpc/server";
import { provisionDatabase } from "./turso-manager";
...
// Module-level Daytona instance (initialized safely)
...
try {
  if (process.env.DAYTONA_API_KEY) {
    ...
    console.warn(
      "[DaytonaSandboxManager] DAYTONA_API_KEY not set — Daytona client will be unavailable",
    );
  }
} catch (err) {
  console.error("[DaytonaSandboxManager] Daytona initialization error:", err);
  daytona = null;
}
...
export async function getSandbox(
  ...
  try {
    ...
    try {
      if (!daytona) {
        console.log(
          "[DaytonaSandboxManager] Daytona client not initialized - cannot verify sandbox",
        );
        return null;
      }
...
async function createSandboxInternal(
  ...
  try {
    ...
    if (!daytona) {
      throw new Error(
        "Daytona client not initialized - DAYTONA_API_KEY environment variable may be missing",
      );
    }
...
export async function startSandbox(sandboxId: string): Promise<Sandbox | null> {
  try {
    ...
    if (!daytona) {
      console.log(
        "[DaytonaSandboxManager] Daytona client not initialized - cannot start sandbox",
      );
      return null;
    }
